#include "btint.hpp"
    ...
    template <size_t T>
        ...
        const btint<T> m_a[X_DIMENSION][Y_DIMENSION] = {
            ...
            {btint<T>().from_int(0), btint<T>().from_int(2), btint<T>().from_int(4), btint<T>().from_int(3)},
            {btint<T>().from_int(0), btint<T>().from_int(0), btint<T>().from_int(3), btint<T>().from_int(4)}
        };
    #endif
    ...
    template <size_t T>
        ...
        const btint<T> m_b[Y_DIMENSION][X_DIMENSION] = {
            ...
            {btint<T>().from_int(2), btint<T>().from_int(3), btint<T>().from_int(4)},
            {btint<T>().from_int(3), btint<T>().from_int(4), btint<T>().from_int(5)},
            {btint<T>().from_int(4), btint<T>().from_int(5), btint<T>().from_int(6)}
        };
    #endif
    ...
    template <size_t T>
        ...
        sc_biguint<2> op;
    #else
        const sc_biguint<2> op = INVERSION;
    #endif
#endif
