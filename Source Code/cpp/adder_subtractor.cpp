...
void ADDER_SUBTRACTOR<T>::source(void) {
    for(int i = 0; i < T; i++) {
        ...
        input_b[0][i].write(adder_subtractor_b.read().btint_a[i] ^ adder_subtractor_subtract.read());
        input_b[1][i].write(adder_subtractor_b.read().btint_b[i] ^ adder_subtractor_subtract.read());
    }
}
...
void ADDER_SUBTRACTOR<T>::sink(void) {
    ...
    sum = sum.set_overflow(sum.get_value(T));
